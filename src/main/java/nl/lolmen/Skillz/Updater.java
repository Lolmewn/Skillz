    ...
    */     
    ...
    private void saveFile(File folder, String file, String u)
    {
        ...
        try
        {
            ...
            int count;
            if(announce) plugin.getLogger().info("About to download a new version: " + versionTitle);
            long downloaded = 0;
            ...
            this.plugin.newVersion = this.versionActual;
        }
        catch (Exception ex)
        {
    ...
    private boolean versionCheck(String title)
    {
        ...
        {
            String version = Double.toString(plugin.version);
            if(title.split("v").length == 2)
            {
                String remoteVersion = title.split("v")[1].split(" ")[0]; // Get the newest file's version number
                if(hasTag(version) || version.equalsIgnoreCase(remoteVersion) || Double.parseDouble(remoteVersion) <= plugin.version)
                {
                    // We already have the latest version, or this build is tagged for no-update
                    result = Updater.UpdateResult.NO_UPDATE;
                    return false;
                }
                this.versionActual = Double.parseDouble(remoteVersion);
            }
            else
            {
