 ...
 * updates if needed. <p> <b>VERY, VERY IMPORTANT</b>: Because there are no
 * standards for adding auto-update toggles in your plugin's config, this system
 * provides NO CHECK WITH YOUR CONFIG to make sure the user has allowed
 * auto-updating. <br> It is a <b>BUKKIT POLICY</b> that you include a boolean
 * value in your config that prevents the auto-updater from running <b>AT
 * ALL</b>. <br> If you fail to include this option in your config, your plugin
 * will be <b>REJECTED</b> when you attempt to submit it to dev.bukkit.org. <p>
 ...
 * auto-updater. <br> If you are unsure about these rules, please read the
 * plugin submission guidelines: http://goo.gl/8iU5l
 *
 * @author H31IX
 */
public class Updater {
     ...
     * updates in console
     */
    public Updater(Skillz plugin, String slug, File file, UpdateType type, boolean announce) {
        this.plugin = plugin;
        this.type = type;
    ...
    private void saveFile(File folder, String file, String u) {
        ...
        try {
            // Download the file
            URL remote = new URL(u);
            int fileLength = remote.openConnection().getContentLength();
            in = new BufferedInputStream(remote.openStream());
            fout = new FileOutputStream(folder.getAbsolutePath() + "/" + file);

            byte[] data = new byte[BYTE_SIZE];
            int count;
            if (announce) {
                plugin.getLogger().info("About to download a new version: " + versionTitle);
            }
            long downloaded = 0;
            ...
            this.plugin.newVersion = this.version;
        } catch (Exception ex) {
    ...
    private boolean versionCheck(String title) {
        if (type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split("v").length == 2) {
                String remoteVersionString = title.split("v")[1].split(" ")[0]; // Get the newest file's version number
                Version remoteVersion = new Version(remoteVersionString);
                if (hasTag(v.getInput()) || v.getInput().equalsIgnoreCase(remoteVersion.getInput()) || v.getMajor() > remoteVersion.getMajor() || v.getMinor() > remoteVersion.getMinor()) {
